...
try {
    if (
        ...
        $set = [
            ...
            'updated_by' => new ObjectId($adminIdString)
        ];
        ...
        if ($action === 'approve_service_change') {
            $requestedServiceId =
                $order['requested_service_id'] ?? null;

            if (!$requestedServiceId instanceof ObjectId) {
                throw new RuntimeException(
                    'No requested service was found.'
                );
            }
            ...
            $set['weight_kg'] =
                $order['requested_weight_kg'] ??
                $order['weight_kg'] ??
                0;
            $set['total_amount'] =
                $order['requested_total_amount'] ??
                $order['total_amount'] ??
                0;
            ...
            $successMessage =
                "Service change approved for order #" .
                $orderIdText . '.';
...
try {
    ...
    error_log(
        'Pending reservation count error: ' .
        $e->getMessage()
    );
}
...
for ($index = 0; $index < 7; $index++) {
    ...
    $start = new UTCDateTime(
        $day->setTime(0, 0)->getTimestamp() * 1000
    );
...
<div class="container pb-5">
    ...
    <?php if (
        ...
        <div class="row mb-4">
            <?php if ($cancelRequests > 0): ?>
                <div class="col-md-6">
                    <div class="alert alert-warning">
                        🔔 <?= h($cancelRequests) ?>
                        cancellation request(s) waiting for review.
                    </div>
                </div>
            <?php endif; ?>
            ...
            <?php if ($serviceChangeRequests > 0): ?>
                <div class="col-md-6">
                    <div class="alert alert-info">
                        🔄 <?= h($serviceChangeRequests) ?>
                        service change request(s) waiting for review.
                    </div>
                </div>
            <?php endif; ?>
        </div>
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-white">
            <h5 class="mb-0">
                Weekly Customers in Laundry
            </h5>
        </div>

        <div class="card-body">
            <canvas
                id="weeklyCustomerChart"
                height="100"
            ></canvas>
        </div>
    </div>
...
<script>
new Chart(
    document.getElementById('weeklyCustomerChart'),
    {
        type: 'line',
        data: {
